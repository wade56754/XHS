...
export interface XhsAuthorDataType {
  id: string;
  userName: string;
  userId: string;
  subscribers: string;
  ...
  likes: string;
  allTitles: string[];
  top10Notes: XhsNoteType[];
  createdAt: string;
  profileUrl: string;
}

export type XhsAuthorStorageType = BaseStorageType<XhsAuthorDataType[]> & {
  addAuthor: (authorData: Omit<XhsAuthorDataType, 'id' | 'createdAt'>) => Promise<void>;
