...
> 版本: v2.0 | 创建日期: 2024-12-11
> 配套文档: [DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md) | [方案3：模块化Agent设计.md](./方案3：模块化Agent设计.md)
...
1. [计划概述](#1-计划概述)
2. [模块化Agent架构](#2-模块化agent架构)
3. [可复用资源集成策略](#3-可复用资源集成策略)
...
> 基于 Plan-Execute + Multi-Agent 混合模式设计，实现从热点采集到内容发布的全流程自动化
...
│  • 小红书热榜 (DailyHotApi)                                          │
│  • 微博热搜 (hot_topics.py)                                          │
│  • 用户自定义关键词 (飞书 business_config 表)                         │
│  • 行业RSS订阅 (N8N RSS Trigger) [扩展]                              │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
...
│                      存储层 (Storage Layer)                          │
├─────────────────────────────────────────────────────────────────────┤
│  • topic_candidates (选题候选库)                                     │
│  • topic_history (热点历史 - 去重)                                   │
│  • content_records (内容记录)                                        │
│  • business_config (业务配置 - 热更新)                               │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 六大Agent模块设计

#### Agent 1: Collector (热点采集)
...
**N8N节点**: `sub_topic_collector`

#### Agent 2: Evaluator (相关性评估)
...
interface RelevanceScore {
  topic_id: string;
  scores: {
    direction_match: number;    // 内容方向匹配度 (40%)
    audience_fit: number;       // 受众匹配度 (30%)
    keyword_relevance: number;  // 关键词相关性 (20%)
    trend_potential: number;    // 趋势潜力 (10%)
  };
  total_score: number;
...
**N8N节点**: `sub_relevance_evaluator`

#### Agent 3: Planner (选题规划)

```typescript
interface TopicPlannerAgent {
  generateOutline(topic: EnrichedTopic): Promise<TopicOutline>;
...
**N8N节点**: `sub_topic_planner`

#### Agent 4: Outliner (内容大纲生成)
...
```
COLLECTED → EVALUATING ─┬→ RELEVANT → OUTLINE_GENERATED → AI_SCORED ─┬→ APPROVED → IN_CONTENT_QUEUE
...
| 维度 | 权重 | 说明 |
|------|------|------|
| 内容方向匹配度 | 40% | 是否属于AI/自动化/效率工具方向 |
| 目标受众匹配度 | 30% | 是否对目标用户有吸引力 |
| 关键词相关性 | 20% | 与业务关键词的重合度 |
| 趋势潜力 | 10% | 是否有持续讨论价值 |
...
| 维度 | 权重 | 满分 | 评估要点 |
|------|------|------|----------|
| 点击力 | 30% | 30分 | 标题吸引力(15) + 开头钩子(15) |
| 内容力 | 25% | 25分 | 干货密度(10) + 逻辑性(8) + 可操作性(7) |
| 价值感 | 20% | 20分 | 问题解决能力(10) + 独特性(10) |
| 互动设计 | 10% | 10分 | 评论引导(5) + 收藏动机(5) |
| 平台适配 | 15% | 15分 | 小红书风格(8) + 合规性(7) |

**决策阈值:**
- `score >= 85`: 自动通过 (auto_approve)
- `70 <= score < 85`: 人工审核 (human_review)
- `score < 70`: 自动拒绝 (auto_reject)
...
def calculate_priority_score(
    ...
    """
    base_score = ai_score * 0.5 + relevance_score * 0.3 + heat_score * 0.2
...
def calculate_time_decay(hours_since_collected: int) -> float:
    ...
    if hours_since_collected <= 6:
        ...
        return 1.0 - (hours_since_collected - 6) / 18 * 0.3
    elif hours_since_collected <= 48:
        return 0.7 - (hours_since_collected - 24) / 24 * 0.3
    else:
        return 0.4
```
...
| created_at | 日期 | 创建时间 |
| status | 单选 | 状态 (11种) |
| topic_title | 文本 | 原始热点标题 |
| topic_source | 单选 | 来源平台 |
| topic_url | URL | 原始链接 |
| topic_keywords | 多选 | 提取的关键词 |
| relevance_score | 数字 | 相关性总分 |
| relevance_details | 长文本/JSON | 详细评分 |
| planned_titles | 长文本/JSON | 标题备选 |
| content_angle | 文本 | 切入角度 |
| final_title | 文本 | 最终标题 |
| content_outline | 长文本/JSON | 完整大纲 |
| ai_score_total | 数字 | AI评分总分 |
| ai_score_breakdown | 长文本/JSON | 五维度评分 |
| decision | 单选 | 决策结果 |
| priority_score | 数字 | 优先级分数 |
| urgency_level | 单选 | 紧急程度 |
| expiry_date | 日期 | 时效截止 |
| content_record_id | 关联 | 关联内容记录 |
...
|--------|------|------|
| config_key | 文本 | 配置键名 |
| config_value | 长文本 | 配置值 (支持JSON) |
| config_type | 单选 | prompt/rule/keyword/threshold |
| description | 文本 | 配置说明 |
| is_active | 复选框 | 是否启用 |
| updated_at | 日期 | 更新时间 |
...
- key: BUSINESS_CONTEXT
  value: "你是一个专注于AI自动化和工作流优化的小红书创作者..."
  type: prompt
...
#### 3.1.4 飞书 API - feishu-bitable-python-tool
...
### 4.2 Phase 1: MVP 阶段（Day 3-10）

#### 4.2.1 Sprint 1: 核心数据流（Day 3-5）
...
### 4.3 Phase 2: 优化阶段（Day 11-20）

#### 4.3.1 Sprint 4: Prompt 优化（Day 11-13）
...
#### 4.3.2 Sprint 5: 热点匹配增强（Day 14-16）
...
#### 4.3.3 Sprint 6: 错误处理与数据分析（Day 17-20）
...
### 4.4 Phase 3: 扩展阶段（Day 21-40）

#### 4.4.1 Sprint 7: 图片生成（Day 21-25）
...
#### 4.4.2 Sprint 8: MCP 发布集成（Day 26-30）
...
#### 4.4.3 Sprint 9: 多账号与数据回流（Day 31-35）
...
#### 4.4.4 Sprint 10: AI 闭环与最终验收（Day 36-40）
...
### 5.2 MVP 阶段 docker-compose.yml（推荐）
...
**最后更新：** 2025-12-11
**更新说明：**
- v2.0 整合模块化Agent设计（方案3），新增6大Agent模块架构、11状态机设计、5维度评分体系、3张新飞书表结构
- v1.1 加入分阶段集成策略，MVP 阶段简化部署（N8N+Nginx Docker + 热点脚本 Crontab），扩展阶段再集成 MCP (pm2)
